    ...
    return async function(req, res, next) {
        ...
        if (pathname === '/__nextjs_original-stack-frames') {
            ...
            if (isAppRelativePath) {
                ...
                const absoluteFilePath = path.join(projectPath, 'app', isSrcDir ? 'src' : '', relativeFilePath);
                openEditorResult = await openFileInEditor(absoluteFilePath, 1, 1);
            } else {
                const frame = createStackFrame(searchParams);
                if (!frame) return middlewareResponse.badRequest(res);
                openEditorResult = await openFileInEditor(frame.file, frame.line ?? 1, frame.column ?? 1);
            }
